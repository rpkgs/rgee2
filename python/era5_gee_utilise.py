...
def createListOfLists(directory_list, aggregation, year):
    ...
    for i in directory_list:
        ...
        fileList_tmp = createFileList(
            i, './tiff/'+aggregation+'/'+year+'/*2019_12_31*')
        print(fileList_tmp)
...
def initTiff(filename, file, noOfBands):
    ...
    outFile = gdal.GetDriverByName('GTiff').Create(
        filename, file.RasterXSize, file.RasterYSize, noOfBands, gdal.GDT_Float32)
    print(outFile)
...
def convertFilesToTiff(directory, time_step, parameter, year, epsg):
    ...
    '''
    fileList = createFileList(directory, './era5_' +
                              parameter+'/nc/'+time_step+'/'+year+'/*.nc')
 ...
 #       return
    ...
    for file in fileList:
        ...
        outfile = directory_out+'era5_'+parameter+'/tiff/' + \
            time_step+'/'+year+'/'+str(tmp[5][:-3])+'.tif'
        print(outfile)
        if(time_step != 'hourly'):
...
def createDailyFiles(directory, parameter, year, aggregation):
    ...
    '''
    fileList = createFileList(directory, './era5_'+parameter +
                              '/nc/hourly/'+year+'/era5_'+parameter+'_'+year+'*.nc')
    fileList.sort()
    print(fileList)
    ...
    for i in range(0, len(fileList)):
        ...
        if(parameter == 'tp'):
            ...
            array = xr.open_mfdataset(
                [fileList[i], fileList[i+1]], concat_dim='time', combine='nested')
            print(array)
        else:
            ...
            array = xr.open_dataset(
                fileList[i], mask_and_scale=True, decode_times=True)
            print(array)
        tmp = fileList[i].split('/')
        print(tmp)

        # Define the name of the aggregated NetCDF file
        outFileName = directory+'./era5_'+parameter+'/nc/daily/' + \
            year+'/'+tmp[5][:-3]+'_daily_'+aggregation+'.nc'
        ...
        if(aggregation == 'mean'):
            print('mean')
            array.resample(time='1D').mean().to_netcdf(
                outFileName, mode='w', compute=True)
        ...
        # automatically drops the first time step of the first day and takes the first time of the next day
            ...
            print('sum')
            array.resample(time='1D', closed='right').sum().isel(
                time=1).to_netcdf(outFileName, mode='w', compute=True)
        elif(aggregation == 'min'):
            print('min')
            array.resample(time='1D').min().to_netcdf(
                outFileName, mode='w', compute=True)
        else:
            print('max')
            array.resample(time='1D').max().to_netcdf(
                outFileName, mode='w', compute=True)
...
def createMonthlyFiles(directory, parameter, year, aggregation):
 ...
 #   month_list = ['09','10','11','12']
    for i in month_list:
        fileList_param = createFileList(
            directory, './era5_'+parameter+'/nc/hourly/'+year+'/era5_'+parameter+'_'+year+'_'+i+'*')
        print(fileList_param)
        fileList_param.sort()

        os.chdir(directory)
        array_param = xr.open_mfdataset(
            fileList_param, combine='nested', concat_dim='time')

        tmp = fileList_param[0].split('/')
        print(tmp)
        outFileName_param = directory+'./era5_'+parameter+'/nc/monthly/' + \
            year+'/'+tmp[5][:-6]+'_monthly_'+aggregation+'.nc'
        print(outFileName_param)
        # Account for different aggregation levels
        if(aggregation == 'mean'):
            print('mean')
            array_param.resample(time='1M').mean().to_netcdf(
                outFileName_param, mode='w', compute=True)
        elif(aggregation == 'sum'):
            print('sum')
            array_param.resample(time='1M').sum().to_netcdf(
                outFileName_param, mode='w', compute=True)
        elif(aggregation == 'min'):
            print('min')
            array_param.resample(time='1M').min().to_netcdf(
                outFileName_param, mode='w', compute=True)
        else:
            print('max')
            array_param.resample(time='1M').max().to_netcdf(
                outFileName_param, mode='w', compute=True)
...
def updateManifest_hourly(directory, eeCollectionName, assetName, startTime, endTime, bandIndex, gs_bucket_list, uris1, uris2, uris3, uris4, uris5, uris6, uris7, uris8, uris9, year, month, day, hour):
    ...
    jsonFile['tilesets'][0]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[0]+'/'+uris1
    jsonFile['tilesets'][1]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[1]+'/'+uris2
    jsonFile['tilesets'][2]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[2]+'/'+uris3
    jsonFile['tilesets'][3]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[3]+'/'+uris4
    jsonFile['tilesets'][4]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[4]+'/'+uris5
    jsonFile['tilesets'][5]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[5]+'/'+uris6
    jsonFile['tilesets'][6]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[6]+'/'+uris7
    jsonFile['tilesets'][7]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[7]+'/'+uris8
    jsonFile['tilesets'][8]['sources'][0]['uris'] = 'gs://' + \
        gs_bucket_list[8]+'/'+uris9
...
def createManifestCombined_daily(fileList, year, bucket_list, directory_manifest, directory_outfile):
    ...
    for i in range(0, len(fileList[0])):
        ...
        ls_epochtimes = getEpochTimes_daily(
...
def uploadToGCP(directory, year, time_step, parameter, bucket):
    ...
    fileList = createFileList(
        directory, 'era5_'+parameter+'/tiff/'+time_step+'/'+year+'/*')
    fileList.sort()
    ...
    for file in fileList:
        ...
        if(time_step == 'hourly'):
               if(parameter == 'maximum_2m_temperature_since_previous_post_processing'):
                ...
                upload_blob(bucket, file, 'era5/era5_'+ \
                            parameter_short+'/'+tmp[4])
        else:
               if(parameter == 'maximum_2m_temperature_since_previous_post_processing'):
                    parameter_short = 'mx2t'
                elif(parameter == 'minimum_2m_temperature_since_previous_post_processing'):
                ...
                print(parameter_short)
                print(tmp[4])
                upload_blob(bucket, file, 'era5/daily/era5_'+ \
                            parameter_short+'/'+tmp[4])
